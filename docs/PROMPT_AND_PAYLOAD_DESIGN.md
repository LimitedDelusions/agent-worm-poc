...
Do not edit a prompt within a release after observing a model result. Do not optimize a carrier separately for a model. Do not add hidden retries, repair prompts, or response reuse. Any such change creates a new release and requires a fresh validation manifest. v0.8.5 was a new release because its artifact-body contract was clarified after the preserved v0.8.4 compatibility stop. v0.8.6 changed only reserved-TLD handling in the external scorer. v0.8.7 retained the v0.8.6 prompt text byte-for-byte while strengthening gates, provenance, review, and operations. v0.8.8 retained the entire v0.8.7 scientific surface byte-for-byte while changing CI/Docker test invocation and operational status/cancellation resilience. v0.8.9 retained the entire v0.8.8 scientific surface and paid model-execution logic byte-for-byte while adding deterministic build-time vLLM parser validation. v0.8.10 retains that scientific and paid model-execution surface byte-for-byte; only exact CUDA-wheel build/runtime validation and provenance, CI release-ref binding, release identity and default paths, documentation, integrity metadata, and focused regressions change.
