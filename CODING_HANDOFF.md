...
Expand-Archive .\agent_worm_poc_v0.8.10.zip .\agent-worm-poc-v0810
Set-Location .\agent-worm-poc-v0810\agent_worm_poc_v0.8.10
```
...
Do not extract over an older source tree. Preserve only `.git`, then copy v0.8.10 into the repository root.
...
python scripts\release\verify_evidence.py $zip.FullName --expected-version 0.8.10
```
...
```powershell
git add -A
git commit -m "Agent worm POC v0.8.10 exact CUDA wheel validation"
git tag v0.8.10
...
1. Start exactly one run against the immutable tag:
   ```powershell
   gh workflow run validate-and-build.yml --ref v0.8.10
   ```
2. Open GitHub → **Actions** and select **Validate and Build Agent Worm POC**.
3. Select the run for `v0.8.10`; do not dispatch a duplicate while it is active.
